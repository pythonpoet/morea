...
class User {
  ...
  getTeilnehmer() async {
    if (_userMap.containsKey(userMapgroupID)) groupID = _userMap[userMapgroupID];
    else throw "$userMapgroupID has to be non-null";

    if (_userMap.containsKey(userMapVorName)) vorName = _userMap[userMapVorName];
    else throw "$userMapVorName has to be non-null";

    if (_userMap.containsKey(userMapNachName)) nachName = _userMap[userMapNachName];
    else throw "$userMapNachName has to be non-null";

    if (_userMap.containsKey(userMapEmail)) email = _userMap[userMapEmail];
    else throw "$userMapEmail has to be non-null";

    if (_userMap.containsKey(userMapAdresse)) adresse = _userMap[userMapAdresse];
    else throw "$userMapAdresse has to be non-null";

    if (_userMap.containsKey(userMapGeburtstag)) geburtstag = _userMap[userMapGeburtstag];
    else throw "$userMapGeburtstag has to be non-null"; 

    if (_userMap.containsKey(userMapOrt)) ort = _userMap[userMapOrt];
    else throw "$userMapOrt has to be non-null";

    if (_userMap.containsKey(userMapPLZ)) plz = _userMap[userMapPLZ];
    else throw "$userMapPLZ has to be non-null";

    if (_userMap.containsKey(userMapUID)) userID = _userMap[userMapUID];
    else throw "$userMapUID has to be non-null";

    if (_userMap.containsKey(userMapGeschlecht)) geschlecht = _userMap[userMapGeschlecht];
    else throw "$userMapGeschlecht has to be non-null";

    if (_userMap.containsKey(userMapHandynummer)) handynummer = _userMap[userMapHandynummer];
    //Handynummer can be empty (Because children without a Mobilephone are able to register).

    if (_userMap.containsKey(userMapSubscribedGroups)) subscribedGroups = List<String>.from(_userMap[userMapSubscribedGroups]);
  ...
  getElterData() async { 
    ...
    if (_userMap.containsKey(userMapHandynummer)) handynummer = _userMap[userMapHandynummer];
    else throw "$userMapHandynummer has to be non-null";
  ...
  Map<String, dynamic> generateAndValitateUserMap() {
    ...
    if(adresse != null) userMap[userMapAdresse] = adresse;
    else throw "$userMapAdresse can't be null";

    if(email != null)
    userMap[userMapEmail] = email;
    else throw "$userMapEmail has to be non-null";

    if(vorName != null) userMap[userMapVorName] = vorName;
    else throw "$userMapVorName has to be non-null";

    if(nachName != null) userMap[userMapNachName] = nachName;
    else throw "$userMapNachName has to be non-null";

    if(userID != null) userMap[userMapUID] = userID;
    else throw "$userMapUID has to be non-null";

    if(ort != null) userMap[userMapOrt] = ort;
    else throw "$userMapOrt has to be non-null";

    if(plz != null) userMap[userMapPLZ] = plz;
    else throw "$userMapPLZ has to be non-null";

    if(pos != null) userMap[userMapPos] = pos;
    else throw "$userMapPos has to be non-null";
    ...
    switch (pos) {
      case "Teilnehmer":
        ...
        if(geschlecht != null) userMap[userMapGeschlecht] = geschlecht;
        else throw "$userMapGeschlecht has to be non-null";
