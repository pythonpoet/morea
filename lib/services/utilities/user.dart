...
class User {
  String displayName,pfadiName, groupID, vorName, nachName, pos, email, adresse, geburtstag, handynummer, ort, plz, userID;
  ...
  Future getUserData(Map<String,dynamic> userMap)async{
    ...
    if ((pos == userMapLeiter) || (pos == userMapTeilnehmer)) {
      ...
      groupPrivilege[groupID] = groupMap["Priviledge"][_userMap[userMapUID]]["Priviledge"];
      else groupPrivilege[groupID] = 0;
...
Map<String, dynamic> generateUserMap(){
  ...
  if(pfadiName != null)
    userMap[userMapPfadiName] = pfadiName;
