...
abstract class BaseTeleblitz{
 ...
 class Teleblitz implements BaseTeleblitz{
  ...
  if(await auth.refreshteleblitz(stufe)== true){
    data = await http.get("https://api.webflow.com/collections/5be4a9a6dbcc0a24d7cb0ee9/items?api_version=1.0.0&access_token=d9097840d357b02bd934ba7d9c52c595e6940273e940816a35062fe99e69a2de");
  ...
  Container getAntreten(){
    if((keineaktivitat== false)||(this?.antreten?.isNotEmpty ?? false)){
      ...
      return Container(
          ...
          child: Row(
            children: <Widget>[
              SizedBox(child: Text('Keine Aktivität',style: TextStyle(fontSize: 30),),),
  ...
  Container getMitnehmen(){
    if((keineaktivitat== false)||(this?.antreten?.isNotEmpty ?? false)){
      return Container(
        ...
        child: Row(
          children: <Widget>[
            SizedBox(width: this._sizeleft,child: Text("Mitnehmen", style: this._getStyleLeft())),
            Expanded(child: Html(data: this.mitnehmen, defaultTextStyle: _getStyleRight(),))
          ],
        ),
      );
    }else{
